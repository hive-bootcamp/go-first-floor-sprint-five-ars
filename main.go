// Пакет main является основным пакетом и отсюда идет запуск проекта.
package main
...
func (t Training) meanSpeed() float64 {
	...
	trainingDistance := t.distance()
	return trainingDistance / hours
}
...
type InfoMessage struct {
	TrainingType string        // тип тренировки
	Duration     time.Duration //средняя скорость, с которой двигался пользователь
	Distance     float64       // длительность тренировки
	Speed        float64       //расстояние, которое преодолел пользователь
...
func (r Running) Calories() float64 {
	meanSpeedInKm := CaloriesMeanSpeedMultiplier*r.distance() + CaloriesMeanSpeedShift
	return meanSpeedInKm * r.Weight / MInKm * r.Duration.Hours() * MinInHours
}
...
func (r Running) TrainingInfo() InfoMessage {
	...
	trainingDistance := r.distance()
	trainingSpeed := r.meanSpeed()
	trainingCalories := r.Calories()
...
func (w Walking) Calories() float64 {
	...
	heightInMtr := w.Height / CmInM
	meanSpeedAndHeight := math.Pow(walkingMeanSpeed, 2) / heightInMtr
	return (CaloriesWeightMultiplier*w.Weight + meanSpeedAndHeight*
...
func (w Walking) TrainingInfo() InfoMessage {
	...
	trainingDistance := w.distance()
	trainingSpeed := w.meanSpeed()
	trainingCalories := w.Calories()
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	trainingDistance := s.distance()
	trainingSpeed := s.meanSpeed()
	trainingCalories := s.Calories()
